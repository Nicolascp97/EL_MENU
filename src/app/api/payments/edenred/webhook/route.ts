...
/**
 * Webhook de Edenred Chile.
 *
 * Este endpoint se configura como "callbackUrl" al crear la transacción.
 * Edenred puede invocar tanto GET (redirect del usuario) como POST
 * (notificación server-to-server). Ambos se manejan aquí.
 *
 * TODO: ajusta los nombres de campos según la documentación real de Edenred:
 *  - Nombre del campo que contiene el orderId ("externalReference", "reference", etc.)
 *  - Nombre del campo de estado ("status", "paymentStatus", etc.)
 *  - Valor para aprobado ("SUCCESS", "APPROVED", "OK", etc.)
 *  - Header de firma ("x-edenred-signature", "x-signature", etc.)
 */
async function handleWebhook(req: NextRequest): Promise<Response> {
  ...
  if (req.method === 'POST') {
    ...
    if (contentType.includes('application/json')) {
      ...
      const data = JSON.parse(rawBody) as { externalReference?: string; status?: string }
      // TODO: ajusta el nombre del campo según la API real de Edenred
      orderId = data.externalReference ?? null
      approved = data.status?.toUpperCase() === 'SUCCESS'
    } else {
      // form-encoded
      const form = await req.formData()
      orderId = (form.get('externalReference') as string) ?? null
      approved = (form.get('status') as string)?.toUpperCase() === 'SUCCESS'
    }
  } else {
    // GET — redirect del navegador del usuario
    const url = new URL(req.url)
    orderId = url.searchParams.get('externalReference')
    approved = url.searchParams.get('status')?.toUpperCase() === 'SUCCESS'
  }
  ...
  if (clientSecret && req.method === 'POST' && rawBody) {
    // TODO: ajusta el header de firma según la documentación de Edenred
    const receivedSig = req.headers.get('x-edenred-signature') ?? ''
    if (receivedSig) {
      ...
      if (receivedSig !== expectedSig) {
        console.error('[Edenred] Firma de webhook inválida')
